#!pip install pennylane autograd scipy matplotlib
...
D = 0.02
Lx = 1.0            # 공간 구간 길이
K  = 4              # 보존할 푸리에-사인 모드 수 (== 2^n 로 두자)
n  = 2              # qubits -> dim=4
assert 2**n == K, "지금 코드는 2^n = K 로 가정"

# wave numbers kappa_k = 2πk / Lx  (k=1..K)
...
for t in ts:
    ...
    psi_hat  = alpha * psi_norm    # 모드 계수 추정치 (길이 4)
...
# 정확해(계수): a_k(t) = a_k(0) e^{lambda_k t}, a(0)=[1,0,0,0]
